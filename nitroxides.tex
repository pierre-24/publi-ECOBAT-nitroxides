...
In 2018, a simple model have been proposed by Zhang \textit{et al} \cite{zhangEffectHeteroatomFunctionality2018} in which change in oxidation potential in nitroxide compounds is explained by the electrostatic interaction between the substituent(s) and the charge formed upon oxidation or reduction.  In first approximation, charge-dipole interactions are stabilizing the oxiamonium ( $>$\ce{N+=O}) if the dipole is aligned with the charge, while this destabilize the hydroxylamine ( $>$\ce{N-O-}), both resulting in a decrease of the redox potential (Fig.~\ref{fig:dipole}).
The electrostatic interaction may be further expanded as a  multipole expansion, which they proposed to truncated after third order, to include the large quadrupole moment of aromatic compounds:\begin{equation}
	U_q(r) = \frac{\mu_x}{r^2} + \frac{Q_{xx}}{r^3}, \label{eq:Er}
\end{equation}
assuming a non-charged substituent. The different quantities (dipole moment, $\mu_x$, and traceless quadrupole moment, $Q_{xx}$) are evaluated through a single point calculation in gas phase on a simplified structure, using the geometry of the radical but where the $>$\ce{N-O^.} moiety is substituted by \ce{CH_2} (the rest of the geometry is kept fixed).  Since the alignment of the dipole with the charge need to be evaluated, this geometry is oriented so that the $x$ axis pass through origin and the nitrogen, the origin being placed at the carbon bearing the substituent. $r$ is the origin-nitrogen distance. Note that this definition for the origin is different from the original model, since  Zhang and co-workers did not consider multiple positions for a given substituent. 
...
To model the impact of the electrolyte on the redox potential, the formation of ion pairs is considered (Fig.~\ref{fig:cip}).  Here, the electrolyte is composed of a pair \ce{AC}, where \ce{A-} and \ce{C+} are a cation and an ion, respectively.  Being favored by electrostatic interactions, the close-contact pairing of the oxidized (\ce{N+}) and reduced (\ce{N-}) state of nitroxide (\ce{N^.}) with its corresponding counterion (\ce{A-} and  \ce{C+},  respectively) is first considered. Then, further complexion with the \ce{AC} pair in close contact is considered  \cite{wylieImprovedPerformanceAllOrganic2019a}.
...
xxx Fig.~S2, but $K_{x1}$ has more impact than $K_{x2}$ (behavior at $[X]\to\infty$? Ratio between the two?).

\subsection{Model for the ion-pair formation}

Insight in the formation of ion pairs of opposite charges ($K_{01}$ and $K_{21}$ in Fig.~\ref{fig:cip}) is provided by a simple model proposed by Lund et al. \cite{lundDielectricInterpretationSpecificity2010}. It is based on the balance between the solvatation of the individual charges [described by the Born model, Eq.~\eqref{eq:born}] and the formation of a dipole when the two charges are in interaction. From the thermodynamic cycle given in Fig.~\ref{fig:ionpair}:\begin{equation}
	\Delta G_{pair}^\star = \frac{1}{4\pi\varepsilon_0}\,\left\{\left[\frac{q_1^2}{2a_1}+\frac{q_2^2}{2a_2}\right]\,\left[1-\frac{1}{\varepsilon_r}\right]+\frac{q_1\,q_2}{a_1+a_2}-\frac{\varepsilon_r-1}{2\varepsilon_r+1}\,\frac{\mu^2}{a^3}\right\},
\end{equation}
where $a_1$, $a_2$ and $a$ are the radius of the solvatation shells for $q_1$, $q_2$, and the dipole, respectively, defined as $\mu = \frac{1}{2}\,|q_2-q_1|\,(a_1+a_2)$. It is assumed that$a=s\,( a_1^3+a_2^3)^{1/3}$ where $s$ is a scaling factor, which accounts from the fact that the shell might not be spherical ($s\geq 1$).
...
\begin{figure}[!h]
	...
	\begin{tikzpicture}
		...
		\draw[latex-latex](4.4,0) -- (5.6,0) node[midway,above]{$a_1+a_2$};
...
This qualitative model (Fig.~S3) shows that the pairing depends on three variables: \begin{inparaenum}[(i)]
	\item the ratio, $\chi = a_1 / a_2$, with $\chi \sim 1$ giving the lowest pair formation energy (which was already pointed out by Lund and co-workers), 
	\item the shape of the final cavity (pairing energy increases with $s$), and
	\item the dielectric constant $\varepsilon_r$.
\end{inparaenum}
This latter parameter only has a minor influence, but pair formation is favored in less polar solvents, as expected.\todo{Le modele est intéréssant pour les liquides ioniques, soit dit en passant}
...
\begin{itemize}
	...
	\item Absolute to relative potentials (?)
\end{itemize}

...
\end{equation}
where $n_i$ is the number density ($n_i = N_i / V = c_i\,\mathcal{N}_a$ where $\mathcal{N}_a$ is the Avogadro number and $c_i$ is the concentration in ion $i$) of ion of type $i$, $k_B$ is the Boltzmann constant, and $T$ is the temperature.  $\kappa$ is  proportional to the ionic strength of the solution, $I = \frac{1}{2}\sum_i c_i\,z_i^2$. 
...
In the limit of $\kappa\to 0$,  $\Delta G^\star_{DH} = 0$ and thus $\Delta G^\star_S \approx \Delta G^\star_{born} = \Delta G_d + \Delta G_c$.  Therefore, by combining Eqs.~\eqref{eq:scrf} and \eqref{eq:adh}, one defines:\begin{equation}
	G^\star(X^z) = G^\star_{SCRF}(X^z) + \Delta G^\star_{DH}(X^z),
\end{equation}
...
\begin{itemize}
	\item Structure-activity relationships, on simple results (Fig.~\ref{fig:family}), if possible (see Hodgson et al \cite{hodgsonOneElectronOxidationReduction2007}). Also, comparing water and acetonitrile (Fig.~\ref{fig:watvsac}).
	\item low-concentration limit: DH corrections
	\item High-concentration limit: CIP (also, structure-activity relationship for CIP!) and Matsui.
...
\caption{Relationship between absolute oxidation (top) and reduction (bottom) potentials of nitroxides and the electrostatic potential between the redox center ($>$\ce{N-O^.}) and the substituent, as computed at the $\omega$B97X-D/6-311+G(d) level in water (SMD), with $[\ce{X}]=\SI{0}{\mole\per\liter}$.}
\label{fig:corr}
\end{figure}
...
\begin{figure}[!h]
	...
	\caption{Impact of the Debye-Huckel correction, as $\Delta E^0 = -\frac{\Delta G_{DH}^\star}{F}$ for $[X]=\SI{1}{\mole\per\liter}$ (round markers), $[X]=\SI{0.1}{\mole\per\liter}$ (triangular markers)  and $[\ce{X}]=\SI{0}{\mole\per\liter}$ (square markers), as computed at the $\omega$B97X-D/6-311+G(d) level in water (top) and acetonitrile (bottom) using SMD. Filled (empty) markers represent the correction to the oxidation (reduction) potential. }
	\label{fig:DH}
\end{figure}

...
An example is provided in Fig.~S2:  in order to have an impact two conditions should be met: \begin{inparaenum}[(i)]
	\item the complexation constant should be large, and
	\item there should be a large difference between the complexation energy of one species with respect to the other in the redox couple.
\end{inparaenum}
...
\begin{itemize}
	...
	\item What about Matsui? (Fig.~\ref{fig:matsui})
\end{itemize}
...
\begin{figure}[!h]
	...
	\caption{Comparison between experimental ($E^0_{rel} $ vs SHE, from Table S8) and corrected oxidation potential using the scheme of Matsui et al. \cite{matsuiDensityFunctionalTheory2013} [Eq.~\eqref{eq:matsui}, with the parameter $E_{SHE}$, $f$, and $\mu$ obtained by fitting procedure], as computed at the $\omega$B97X-D/6-311+G(d) level in water (left) and acetonitrile (right).  }
	\label{fig:matsui}
\end{figure}
